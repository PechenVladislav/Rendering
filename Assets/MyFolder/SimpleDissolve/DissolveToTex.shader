	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float second = step(_AlphaCuroff, dissolve);
				float edge = second * step(dissolve - _EdgeWidth, _AlphaCuroff);

				float3 edgeTex = tex1D(_EdgeRamp, 1 - (dissolve - _AlphaCuroff) / _EdgeWidth) * _EdgeTint;
				col.rgb =  col.rgb * (1 - edge - (1 - second)) + edgeTex * edge + secondCol * (1 - second);
